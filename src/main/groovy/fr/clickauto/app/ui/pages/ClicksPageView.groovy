...
final class ClicksPageView {
    ...
    private Node createPage() {
        ...
        page.setTop(createToolbar(actionList))
        page.setCenter(createCenterPane(actionList))
        page.setRight(createDetailsPane())
    ...
    private Node createToolbar(ListView<Action> actionList) {
        ...
        startButton.setOnAction({ ActionEvent ignored ->
            statusUpdater.accept('Sequence prete a etre lancee')
        } as EventHandler<ActionEvent>)
        pauseButton.setOnAction({ ActionEvent ignored ->
            statusUpdater.accept('Pause demandee')
        } as EventHandler<ActionEvent>)
        stopButton.setOnAction({ ActionEvent ignored ->
            statusUpdater.accept('Arret demande')
        } as EventHandler<ActionEvent>)
        addClickButton.setOnAction({ ActionEvent ignored ->
            addAction(new ClickAction(ClickType.LEFT, 250L, 420, 320))
            actionList.refresh()
            selectLastAction(actionList)
            statusUpdater.accept('Action clic ajoutee')
        } as EventHandler<ActionEvent>)
        addKeyButton.setOnAction({ ActionEvent ignored ->
            addAction(new KeyAction(KeyEvent.VK_A, 120L, 500L))
            actionList.refresh()
            selectLastAction(actionList)
            statusUpdater.accept('Action clavier ajoutee')
        } as EventHandler<ActionEvent>)

        return new ToolBar(startButton, pauseButton, stopButton, addClickButton, addKeyButton)
    }

    private Node createCenterPane(ListView<Action> actionList) {
        Label title = new Label('Sequence en cours')
        title.setFont(Font.font('System', FontWeight.BOLD, 16))

        Label subtitle = new Label('Base de la timeline des actions a executer')
    ...
    private Node createDetailsPane() {
        Label title = new Label('Proprietes')
        title.setFont(Font.font('System', FontWeight.BOLD, 16))
        ...
        addPropertyRow(grid, 2, 'Delai initial', initialDelayValue)
        addPropertyRow(grid, 3, 'Duree totale', totalDurationValue)
        addPropertyRow(grid, 4, 'Actions', actionCountValue)

        Separator separator = new Separator()
        Label selectedTitle = new Label('Action selectionnee')
        selectedTitle.setFont(Font.font('System', FontWeight.BOLD, 13))

        detailsArea.setEditable(false)
        detailsArea.setWrapText(true)
        detailsArea.setPrefRowCount(8)
        detailsArea.setText('Selectionne une action dans la liste pour afficher ses details.')
    ...
    private ListView<Action> createActionList() {
        ...
        actionList.setPlaceholder(new Label('Aucune action dans la sequence.'))
            ...
            new ListCell<Action>() {
                ...
                protected void updateItem(Action item, boolean empty) {
                    ...
                    if (empty || item == null) {
                        ...
                        setText("${item.getActionType()} - ${item.getDescription()}  | delai ${item.getDelayMs()} ms")
                    }
                }
            }
        }
    ...
    private void selectAction(Action action) {
        if (action == null) {
            selectedActionLabel.setText('Aucune action selectionnee')
            detailsArea.setText('Selectionne une action dans la liste pour afficher ses details.')
            return
        }
        ...
        detailsArea.setText(
                "Type: ${action.getActionType()}\n" +
                        ...
                        "Delai avant action: ${action.getDelayMs()} ms\n" +
    ...
    private static void addPropertyRow(GridPane grid, int rowIndex, String labelText, Label valueLabel) {
    ...
    private static Sequence createDemoSequence() {
        Sequence demo = new Sequence('Sequence de demonstration')
        demo.setCycles(3)
